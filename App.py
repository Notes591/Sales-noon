...
import requests
import streamlit.components.v1 as components  # ✅ مضاف
...
if search:
    ...
    df = df[
        df["partner_sku"].astype(str).str.contains(search, case=False, na=False) |
        df["unified_code"].astype(str).str.contains(search, case=False, na=False)
    ]
